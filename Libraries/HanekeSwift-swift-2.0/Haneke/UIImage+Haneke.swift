...
extension UIImage {
    ...
    func hnk_data(_ compressionQuality: Float = 1.0) -> Data! {
    ...
    func hnk_decompressedImage() -> UIImage! {
        ...
        if let context = CGContext(data: nil, width: Int(ceil(pixelSize.width)), height: Int(ceil(pixelSize.height)), bitsPerComponent: (originalImageRef?.bitsPerComponent)!, bytesPerRow: 0, space: colorSpace, bitmapInfo: (bitmapInfo?.rawValue)!) {
