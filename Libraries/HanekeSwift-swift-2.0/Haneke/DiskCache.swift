...
open class DiskCache {
    ...
    open class func basePath() -> String {
        let cachesPath = NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.cachesDirectory, FileManager.SearchPathDomainMask.userDomainMask, true)[0] as String
    ...
    open func fetchData(_ key : String, failure fail : ((NSError?) -> ())? = nil, success succeed : @escaping (Data) -> ()) {
        cacheQueue.async(execute: {
    ...
    open func updateAccessDate( _ getData : @autoclosure @escaping () -> Data?, key : String) {
        cacheQueue.async(execute: {
            let path = self.pathForKey(key)
            let fileManager = FileManager.default
            if (!self.updateDiskAccessDateAtPath(path) && !fileManager.fileExists(atPath: path)){
                if let data = getData() {
                    self.setDataSync(data, key: key)
                } else {
                    Log.error("Failed to get data for key \(key)")
    ...
    fileprivate func setDataSync(_ data: Data, key : String) {
        ...
        do {
            previousAttributes = try fileManager.attributesOfItem(atPath: path) as NSDictionary?
        } catch _ {
    ...
    fileprivate func removeFileAtPath(_ path:String) {
        ...
        do {
            ...
            if isNoSuchFileError(error as NSError) {
